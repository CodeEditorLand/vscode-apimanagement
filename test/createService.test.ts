...
suite(
	...
	async function (this: ISuiteCallbackContext): Promise<void> {
		...
		suiteSetup(async function (this: IHookCallbackContext): Promise<void> {
			...
			const rootNode: AzureParentTreeItem = await treeUtils.getRootNode(
				ext.tree
			);
		...
		suiteTeardown(async function (
			...
			const client: ResourceManagementClient =
				getResourceManagementClient(testAccount);
			for (const resourceGroup of resourceGroupsToDelete) {
				if (
					(await client.resourceGroups.checkExistence(
						resourceGroup
					)) !== undefined
				) {
					console.log(

...
suite("Open API Parser", () => {
	test("Parse OpenAPI Json 2.0", async () => {
		const parsedOpenAPI: IOpenApiImportObject =
			await new OpenApiParser().parse(openApi2_0);
	...
	test("Parse OpenAPI Json > 3.0", async () => {
		const parsedOpenAPI: IOpenApiImportObject =
			await new OpenApiParser().parse(openApi3_0);
	...
	test("Invalid OpenAPI input", async () => {
		...
		await assertThrowsAsync(
			async () => new OpenApiParser().parse(invalidSwagger),

...
import {
	...
	askId,
} from "../commands/authorizations/common";
...
export class AuthorizationProvidersTreeItem extends AzureParentTreeItem<IServiceTreeRoot> {
	...
	public static contextValue = "azureApiManagementAuthorizationProviders";
	public readonly childTypeLabel: string = localize(
		"azureApiManagement.AuthorizationProvider",
		"AuthorizationProvider",
	);
	public label = "Authorizations (preview)";
	...
	public async loadMoreChildrenImpl(
		...
		const tokenProviders: IAuthorizationProviderContract[] =
			await this.apimService.listAuthorizationProviders();

		return this.createTreeItemsWithErrorHandling(
			tokenProviders,
			"invalidApiManagementAuthorizationProvider",
			async (authorizationProvider: IAuthorizationProviderContract) =>
				new AuthorizationProviderTreeItem(this, authorizationProvider),
			...
			return window.withProgress(
				{
					...
					title: localize(
						...
						`Creating Authorization provider '${context.name}' in service ${this.root.serviceName} ...`,
					),
					cancellable: false,
				},
						...
						if (authorizationProvider === undefined) {
							...
							throw new Error(
								localize(
									...
									`Authorization provider '${authorizationProviderName}' already exists.`,
								),
							);
						}
					} catch (error) {
	...
	private async checkManagedIdentityEnabled(): Promise<void> {
		...
		if (service.identity === undefined) {
			...
			const option = await ext.ui.showQuickPick(
				...
				{
					placeHolder: "Enable system assigned managed identity",
					canPickMany: false,
				},
			);
	...
	private async buildContext(
		...
		let supportedIdentityProviders: ITokenStoreIdentityProviderContract[] =
			await this.apimService.listTokenStoreIdentityProviders();
		// tslint:disable-next-line:no-function-expression
		supportedIdentityProviders = supportedIdentityProviders.sort(
			function compare(
				a: ITokenStoreIdentityProviderContract,
				b: ITokenStoreIdentityProviderContract,
			): number {
		...
		const identityProviderPicked = await ext.ui.showQuickPick(
			supportedIdentityProviders.map((s) => {
				return {
					label: s.properties.displayName,
					description: "",
					detail: "",
				};
			}),
		...
		if (
			...
			const grantTypeValue: IGrantTypesContract = <IGrantTypesContract>(
				grantType
			);

			// tslint:disable-next-line:no-any
			// tslint:disable-next-line:no-unsafe-any
			const grant: ITokenStoreGrantTypeParameterContract =
				selectedIdentityProvider?.properties.oauth2.grantTypes[
					grantType
				];

			const parameterValues =
				await askAuthorizationProviderParameterValues(grant);
			...
			if (grantTypeValue === IGrantTypesContract.authorizationCode) {
				...
				grantTypeValue === IGrantTypesContract.clientCredentials
			) {

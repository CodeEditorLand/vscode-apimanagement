...
export class ServiceTreeItem extends AzureParentTreeItem<IServiceTreeRoot> {
	...
	constructor(
		...
		this.authorizationProvidersTreeItem =
			new AuthorizationProvidersTreeItem(this);
	}
	...
	public async loadMoreChildrenImpl(): Promise<
		AzureTreeItem<IServiceTreeRoot>[]
	> {
		if (this.gatewaysTreeItem === undefined) {
			return [
				this.apisTreeItem,
				this.namedValuesTreeItem,
				this.productsTreeItem,
				this.servicePolicyTreeItem,
				this.subscriptionsTreeItem,
				this.authorizationProvidersTreeItem,
			];
		}
		return [

...
export async function pickWebApp(apiApps: Site[]): Promise<Site> {
	// Pick function app
	const apiApp = await ext.ui.showQuickPick(
		apiApps.map((s) => {
...
export async function setAppBackendEntity(
...
async function createApiWithWildCardOperations(
	context: IActionContext & Partial<IApiTreeItemContext>,
	node: ApisTreeItem,
	webAppName: string,
	apiName: string,
...
async function assignAppDataToOperation(
...
function getBsonObjectId(): string {
	...
	return (
		timestamp +
		"xxxxxxxxxxxxxxxx".replace(/[x]/g, getRandomHex()).toLowerCase()
	);
}
...
function generateUniqueOperationNameSubfix(

...
function parseParameter(param: string): ParameterContract {
	...
	const parameter: ParameterContract = {
		name: nameAndType[0].startsWith("*")
			? nameAndType[0].substr(1)
			: nameAndType[0],
	...
	if (defaultValue.length > 1) {
		parameter.defaultValue = defaultValue[1].endsWith("?")
			? defaultValue[1].substr(0, defaultValue[1].length - 1)
			: defaultValue[1];
	}
